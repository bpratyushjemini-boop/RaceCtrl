...
export const F1Coordinator = {
   ...
   * Uses OpenF1 HTTP API exclusively (no Python dependency).
   */
  async getSessionData(year: number, round: number, sessionLabel: string): Promise<any> {
    try {
      console.log(`[F1Coordinator] Attempting OpenF1 for session data: ${year} Round ${round} ${sessionLabel}`);
      const data = await openf1.fetch("session", year, round, sessionLabel);
      if (data && openf1.validate("session", data)) {
        return data;
      }
    } catch (err) {
      console.warn("[F1Coordinator] OpenF1 session data fetch failed:", err);
    }
   ...
   * Priority: LiveF1 (HTTP) -> OpenF1 -> Cache
   */
