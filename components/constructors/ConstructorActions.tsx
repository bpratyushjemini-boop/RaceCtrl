...
export function ConstructorActions({ constructorId, constructorName }: ConstructorActionsProps) {
  const { isFavoriteCategory, toggleFavoriteCategory, mounted } = useFavorites([]);
  const [copied, setCopied] = useState(false);

  const isFav = isFavoriteCategory("teams", constructorId);

  const handleShare = async () => {
    const shareData = {
      title: `${constructorName} · RaceCtrl`,
      text: `Check out ${constructorName}'s team stats on RaceCtrl, your Formula 1 companion.`,
  ...
  return (
    <div className="flex items-center gap-2.5 shrink-0 select-none">
      ...
      <button
        type="button"
        onClick={() => toggleFavoriteCategory("teams", constructorId)}
        ...
        aria-label={isFav ? `Unfollow ${constructorName}` : `Follow ${constructorName}`}
      >
        <svg
      ...
      <button
        ...
        aria-label={`Share profile link for ${constructorName}`}
      >
        {copied ? (
